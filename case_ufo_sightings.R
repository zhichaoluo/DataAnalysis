...
ufo <- read.delim("exercise/Exercise2/data/ufo_awesome.tsv",
...
ggsave(plot = quick.hist,
       filename = file.path("image", "quick_hist.pdf"),
       height = 6,
       width = 8)
...
ggsave(plot = new.hist,
       filename = file.path("image", "new_hist.pdf"),
       height = 6,
       width = 8)
...
ggsave(plot = state.plot,
       filename = file.path("image", "ufo_sightings.pdf"),
       width = 14,
       height = 8.5)


# Create a new graph where the number of signtings is normailzed by the state population
state.pop <- read.csv("exercise/Exercise2/data/census.csv", stringsAsFactors=FALSE)
...
ggsave(plot = state.plot.norm,
     filename = file.path("image", "ufo_sightings_norm.pdf"),
     width = 14,
     height = 8.5)
